...
div.classList.add('divStyles','wrap','collapse','alpha','beta')
...
for (const coursesItem of coursesAndDurationArray) {
    ...
    h1.innerText = coursesItem.title
    p.innerText = 'Month duration'+' '+ coursesItem.monthDuration
    div.append(h1, p)
    document.body.appendChild(div)
}
// ==========================
...
for (const itemSimpson of simpsons) {
    let div = document.createElement('div')
    ...
    p.classList.add('description')
    h1.innerText = itemSimpson.name +' '+itemSimpson.surname+' '+'-'+' '+itemSimpson.age+' '+'year old'
    p.innerText = itemSimpson.info
    img.src = itemSimpson.photo
